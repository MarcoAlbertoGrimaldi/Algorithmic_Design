               ...
               const size_t n)
{
  ...
  // Compute block C11
  // C11 = (P5 + P4) - P2 + P6
  // Sum P5 that is stored in C22
  sum_matrix_blocks(C, C1X, CX1,
                    C, C1X, CX1,
                    C, C2X, CX2,
                    n2);
  // Subtract P2 that is stored in C12
  sub_matrix_blocks(C, C1X, CX1,
                    C, C1X, CX1,
                    C, C1X, CX2,
                    n2);
  // Sum P4 that is stored in C21
  sum_matrix_blocks(C, C1X, CX1,
                    C, C1X, CX1,
                    C, C2X, CX1,
                    n2);
  
  // Store P3 in AUX P3 = S3 x B11
  improved_strassen_aux(AUX, 0, 0,
               S[2], 0, 0,
               B, B1X, BX1,
               n2);

  // Compute block C21 = P3 + P4
  // Sum P3 that is stored in AUX
  sum_matrix_blocks(C, C2X, CX1,
                    C, C2X, CX1,
                    AUX, 0, 0,
                    n2);

  // Subtract P3 from block C22
  sub_matrix_blocks(C, C2X, CX2,
                    C, C2X, CX2,
                    AUX, 0, 0,
                    n2);

  //Store P1 in AUX P1 = A11 x S1
  improved_strassen_aux(AUX, 0, 0,
               A, A1X, AX1,
               S[0], 0, 0,
               n2);

  // Compute block C12 = P1 + P2
  sum_matrix_blocks(C, C1X, CX2,
                    C, C1X, CX2,
                    AUX, 0, 0,
                    n2);
  ...
  // Store P7 in AUX P7 = S9 x S10
  improved_strassen_aux(AUX, 0, 0,
               S[8], 0, 0,
               S[9], 0, 0,
               n2);

  // Compute block C22 = P5 + P1 - P3 - P7
  sub_matrix_blocks(C, C2X, CX2,
                    C, C2X, CX2,
                    AUX, 0, 0,
                    n2);
